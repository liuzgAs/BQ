...
public class BoXingTu extends View {
    ...
    public BoXingTu(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        paintYingYing.setColor(getResources().getColor(R.color.ligthBasic));

        paintText = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintText.setStyle(Paint.Style.FILL);
        paintText.setTextSize(DpUtils.convertDpToPixel(textSize, context));
        paintText.setColor(getResources().getColor(R.color.basicText));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Shader shader = new LinearGradient(widthJianGe * (xuanZhong - 0.5f), 0, widthJianGe * (xuanZhong - 0.5f), height / 2, getResources().getColor(R.color.ligthBasic),
                getResources().getColor(R.color.basic_color), Shader.TileMode.MIRROR);
        ...
        for (int i = 0; i < text.length; i++) {
            if (i == xuanZhong - 1) {
                paintText.setColor(getResources().getColor(R.color.light_black));
            } else {
                paintText.setColor(getResources().getColor(R.color.basic_color));
            }
